...
try {
    ...
    if ($Daily) {
        $metadata = Invoke-RestMethodAuth -Uri 'https://aka.ms/pwsh-buildinfo-daily'
        ...
        $metadata = Invoke-RestMethodAuth -Uri https://raw.githubusercontent.com/PowerShell/PowerShell/master/tools/metadata.json
        if ($Preview) {
